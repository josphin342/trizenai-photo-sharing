...
Stores customer-facing gallery information.
...
Local .env files, node_modules, build output, uploaded files, and other secrets are excluded from Git.
...
- Original S3 objects are not exposed publicly.

This keeps stored photos private while allowing authorized users to view them.

## Gallery Security

Customer galleries are protected using:

1. A unique share token
2. A 6-digit PIN
3. A separate gallery access token after successful PIN verification
...
- Gallery ownership
- Gallery PIN
- Selected photo ownership and event association
...
- Access attempts to unpublished galleries
...
The project includes automated tests for the important authentication, authorization, photo-access, gallery, and PIN-protection workflows.
...
- Runs the Jest test suite using npm test
...
- Creates the production build using npm run build

Deployment is handled separately by:
...
The implementation focuses on the core requirements of the internship challenge.

Optional features not implemented include:
...
These features were treated as optional enhancements so that the core workflow, security, storage, deployment, and testing requirements could remain the primary focus.
...
These credentials are intended for project evaluation. Do not use them for any real account or sensitive service.
